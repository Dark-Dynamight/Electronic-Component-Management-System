import { getFirebaseSync, getCurrentFirebaseSync } from './firebase.js';
// Data management
...
async function initializeSync() {
    ...
    if (!currentUser || !syncEnabled) {
        console.log('Sync disabled or no user logged in');
        return;
    }

    // Generate a unique user ID for Firebase
    const userId = btoa(currentUser.email).replace(/[^a-zA-Z0-9]/g, '');
    firebaseSync = getFirebaseSync(userId);

    // Check connection
    isOnline = await firebaseSync.checkConnection();

    if (isOnline) {
        showNotification('Online sync connected', false);

        // Load data from Firebase
        const cloudData = await firebaseSync.loadData();
        if (cloudData) {
        ...
        firebaseSync.startRealTimeSync(handleCloudUpdate);

        updateSyncStatus();
    } else {
        showNotification('Working offline - changes saved locally', true);
    }
}

async function mergeData(cloudData) {
    // Simple merge strategy: cloud data overwrites local data
    if (cloudData.components && cloudData.components.length > 0) {
        components = cloudData.components;
    }
    ...
    if (cloudData.settings) {
        currency = cloudData.settings.currency || currency;
        lowStockThreshold = cloudData.settings.lowStockThreshold || lowStockThreshold;
...
async function saveToCloud() {
    if (firebaseSync && isOnline) {
        ...
        const success = await firebaseSync.saveData(components, cart, settings);
        if (success) {
...
function handleCloudUpdate(data) {
    if (data) {
        ...
        if (cloudTime > localTime) {
            showNotification('Data updated from cloud', false);
            mergeData(data);
        }
    }
}
...
function updateSyncStatus() {
    ...
    const status = firebaseSync ? firebaseSync.getSyncStatus() : { isOnline: false, lastSync: 'Never' };

    syncStatusEl.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px;">
            <i class="fas ${status.isOnline ? 'fa-cloud' : 'fa-cloud-slash'} 
                     ${status.isOnline ? 'text-success' : 'text-danger'}"></i>
            <span>${status.isOnline ? 'Online' : 'Offline'}</span>
            <small>Last sync: ${status.lastSync}</small>
        </div>
    `;
}
...
function toggleSync(enabled) {
    ...
    if (enabled) {
        ...
        firebaseSync = null;
        showNotification('Cloud sync disabled', true);
    }
...
function updateSyncUI() {
    ...
    if (syncStatus) {
        if (syncEnabled && firebaseSync) {
            updateSyncStatus();
        } else {
            syncStatus.innerHTML = `
                <div style="display: flex; align-items: center; gap: 10px;">
                    <i class="fas fa-cloud-slash text-muted"></i>
...
function setupEventListeners() {
    ...
    document.getElementById('forceSyncBtn')?.addEventListener('click', () => {
        saveToCloud().then(success => {
            if (success) {
                ...
                showNotification('Sync failed - check internet connection', true);
            }
        });
    });
    ...
    document.getElementById('downloadCloudBtn')?.addEventListener('click', async () => {
        if (firebaseSync) {
            ...
            if (cloudData) {
                await mergeData(cloudData);
                showNotification('Data downloaded from cloud');
            } else {
                showNotification('No data found in cloud', true);
            }
        }
    });
}
